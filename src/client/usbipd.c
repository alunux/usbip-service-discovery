...
send_reply_devlist(int connfd)
{
    ...
    list_for_each(j, &host_driver->edev_list) { reply.ndev += 1; }
...
listen_all_addrinfo(struct addrinfo* ai_head, int sockfdlist[], int maxsockfd)
{
    ...
    for (ai = ai_head; ai && nsockfd < maxsockfd; ai = ai->ai_next) {
        ...
        if (sock < 0) {
            err("socket: %s: %d (%s)", ai_buf, errno, strerror(errno));
            continue;
        }
        ...
        if (sock >= FD_SETSIZE) {
            err("FD_SETSIZE: %s: sock=%d, max=%d", ai_buf, sock, FD_SETSIZE);
            close(sock);
            continue;
        }
        ...
        if (ret < 0) {
            err("bind: %s: %d (%s)", ai_buf, errno, strerror(errno));
            close(sock);
            continue;
        }
        ...
        if (ret < 0) {
            err("listen: %s: %d (%s)", ai_buf, errno, strerror(errno));
            close(sock);
            continue;
        }
...
write_pid_file(void)
{
    if (pid_file) {
        ...
        fp = fopen(pid_file, "w");
        if (!fp) {
            err("pid_file: %s: %d (%s)", pid_file, errno, strerror(errno));
            return;
        }
        fprintf(fp, "%d\n", getpid());
        fclose(fp);
...
do_standalone_mode(int daemonize, int ipv4, int ipv6)
{
    ...
    if (usbip_host_driver_open()) {
        err("please load " USBIP_CORE_MOD_NAME ".ko and " USBIP_HOST_DRV_NAME
            ".ko!");
        return -1;
    }
        ...
        if (r < 0) {
            ...
            for (i = 0; i < nsockfd; i++) {
                if (fds[i].revents & POLLIN) {
                    dbg("read event on fd[%d]=%d", i, sockfdlist[i]);
