...
bind_usbip(char* busid)
{
    ...
    if (rc < 0) {
        err("error binding device %s to driver: %s", busid, strerror(errno));
        return -1;
    }
...
unbind_other(char* busid)
{
    ...
    if (!strncmp(USBIP_HOST_DRV_NAME, driver, strlen(USBIP_HOST_DRV_NAME))) {
...
int
usbip_bind(int argc, char* argv[])
{
    static const struct option opts[] = {
        { "busid", required_argument, NULL, 'b' }, { NULL, 0, NULL, 0 }
    };
